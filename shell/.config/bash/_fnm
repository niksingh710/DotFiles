_fnm() {
    ...
    case "${cmd}" in
        fnm)
            opts=" -h -V  --help --version --node-dist-mirror --fnm-dir --multishell-path --log-level --arch   list-remote list install use env completions alias unalias default current exec uninstall help  ls-remote  ls"
        ...
        fnm__ls__remote)
            ...
            ;;
        fnm__unalias)
            opts=" -h -V  --help --version --node-dist-mirror --fnm-dir --log-level --arch  <requested-alias> "
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
